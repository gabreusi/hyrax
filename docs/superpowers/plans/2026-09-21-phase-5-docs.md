...
| **O `npm ci` do Node 22 (npm 10) recusou o lockfile gerado pelo npm 12** (`Missing: react@18.3.1 from lock file`, mais `@types/react@18.3.31`, `react-dom@18.3.1`...), enquanto o npm 11 (Node 24 e 26) e o npm 12 o aceitavam. **O replay do plano só usou o npm 12 e não viu isso: quem viu foi o Node 22 do CI, no primeiro PR** (reproduzido com `npx npm@10 ci --dry-run`) | O lockfile é regenerado com o **npm 10**, o mais antigo com que o CI instala (`npx -y npm@10 install --package-lock-only --ignore-scripts`), e conferido com `npm ci --dry-run` nas versões 10, 11 e 12 (Task 1, Step 3, e Task 8). O CONTRIBUTING registra a regra |
...
- [ ] **Step 3: Regenerar o lockfile com o npm 10.** O npm 10 (o do Node 22) recusa um lockfile escrito por um npm mais novo (veja os achados), então o lockfile é escrito por ele e conferido nas três versões:

```bash
npx -y npm@10 install --package-lock-only --ignore-scripts --no-audit --no-fund
for v in 10 11 12; do npx -y npm@$v ci --dry-run --ignore-scripts --no-audit --no-fund 2>&1 | grep -E "EUSAGE|Missing|added"; done
```

Esperado: as três linhas dizem `added N packages` (562 no npm 10 e 467 nos outros dois), sem `EUSAGE` nem `Missing`.

- [ ] **Step 4: Os scripts.** O `check` ganha os passos da documentação só na Task 7, para não quebrar até lá.
...
- [ ] **Step 5: Os ignores e os `include`.** O `.examples/` (o diretório temporário do verificador), a API gerada e a saída do VitePress ficam fora do git, do Prettier e do ESLint; o `config.ts` do VitePress entra nos dois `tsconfig`. Salve o script abaixo **fora do repositório** e rode-o da raiz.
...
Esperado: `exit=0`, 53 arquivos, 568 testes, `Examples passed: 130 type-checked, 99 of 99 run with 132 assertions`, `build complete`, e o `npm ci --dry-run` das três versões do npm dizendo `added N packages`.
